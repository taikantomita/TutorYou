...
from app import app, get_db, Base, User  # Import directly from app
...
def test_register_duplicate_user():
    # First registration
    client.post("/register", json={"username": "duplicateuser", "password": "password1"})

    # Attempt to register with the same username
    response = client.post("/register", json={"username": "duplicateuser", "password": "password2"})
...
def test_register_blank_fields(username, password):
    ...
    assert response.status_code == 422  # FastAPI automatically handles validation errors for blank fields
...
def test_login_user():
    ...
    response = client.post("/login", json={"username": "loginuser", "password": "loginpassword"})
